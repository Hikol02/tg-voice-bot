...
# Универсальный установщик Telegram Voice Transcriber Userbot
# Автоматически определяет дистрибутив Linux и набор инструкций CPU (Phenom II / AVX / AVX2).
...
read -p "   Choice [1/2/3, default 1]: " LANG_CHOICE
...
if [[ -z "$HAS_AVX2" && -z "$HAS_AVX" ]]; then
    echo "[INFO] AMD Phenom II or non-AVX CPU detected. Building whisper.cpp in pure CPU compatibility mode..."
    CMAKE_FLAGS="-DGGML_AVX=OFF -DGGML_AVX2=OFF -DGGML_FMA=OFF -DGGML_F16C=OFF"
elif [[ -z "$HAS_AVX2" && -n "$HAS_AVX" ]]; then
    echo "[INFO] AVX1 detected without AVX2. Building whisper.cpp with -DGGML_AVX2=OFF..."
...
echo "[INFO] Starting userbot for Telegram authorization..."
